...
namespace view_model_builders
{
    class HelloMessageConsumer : IMessageConsumer
    {
        ...
        public void Start()
        {
            _messageBus.GetIncomingMessageObservable(MessageType.RepositoryPathRequested).Subscribe(new MessageObserver(ResolveRepositoryPath, _messageBus, _log));
        }

        private void ResolveRepositoryPath(Message message)
        {
            var model = message.Body;
